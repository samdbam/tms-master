frappe.ui.form.on("Sales Order", {
    ...
    before_save(frm){
        if(frm.is_new()){
            if(frm.doc.items){
                for(let order of frm.doc.items){
                    order.custom_shipped_qty=0
                    order.custom_sales_order=""
                    order.custom_so_ordered_qty=order.qty
                }
                frm.refresh_field("items")
            }
        }
