...
@frappe.whitelist()
def make_freightify_shipment_am(source_name, target_doc=None, ignore_permissions=False):
...
def check_shipment_shedule_and_rate(doc,doctype):
	...
	if not frappe.db.exists(doctype,{"reference_doctype":"Freightify Shipment Am","reference_name":self.name,"docstatus":["!=",2]}):
		return {"function":"Success"}
	else:
		values=frappe.db.get_value(doctype,{"reference_doctype":"Freightify Shipment Am","reference_name":self.name},["name"])
            ...
            for charge in json.loads(item.charges_json):
                ...
                if len(charges_list)>0:
                    for charge in charges_list:
                        ...
                        if doctype in ["Quotation","Sales Order","Freightify Shipment Am"]:
